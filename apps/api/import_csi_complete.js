...
async function importCSI() {
  ...
  console.log('1️⃣ 正在删除旧的章节数据...');
  const deleted = await knex('template_sections')
    .where('template_id', templateId)
    .del();
  console.log(`   ✅ 已删除 ${deleted} 个旧章节`);
  
  console.log('\n2️⃣ 正在读取新的CSV数据...');
  const csvContent = fs.readFileSync('/tmp/csi_complete_all_levels.csv', 'utf-8');
